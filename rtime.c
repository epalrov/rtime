...
static int rtime_null(CLIENT *cl)
{
	int ret = 0;
	void *arg = NULL;
	void *res;
...
static int rtime_clock(CLIENT *cl, clock_t *clock)
{
	int ret = 0;
	void *arg = NULL;
	rtime_clock_res *res;
...
static int rtime_clock_gettime(CLIENT *cl, clockid_t clock_id,
	struct timespec *tp)
{
	int ret = 0;
...
static int rtime_clock_settime(CLIENT *cl, clockid_t clock_id,
	struct timespec *tp)
{
	int ret = 0;
...
int main(int argc, char *argv[])
{
	...
	case RTIME_CLOCK_GETTIME:
		...
		clock_id = 0;
		ret = rtime_clock_gettime(cl, clock_id, &tp);
		if (!ret)
	...
	case RTIME_CLOCK_SETTIME:
		...
		tp.tv_sec = 10;
		tp.tv_nsec = 10;
		ret = rtime_clock_settime(cl, clock_id, &tp);
		if (ret)
